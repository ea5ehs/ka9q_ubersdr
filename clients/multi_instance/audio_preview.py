...
# Import audio decoders
from audio_decoders import decode_opus_binary, decode_pcm_zstd_binary, OPUS_AVAILABLE, ZSTD_AVAILABLE
...
class AudioChannel:
    ...
    def __init__(self, channel_name: str):
        ...
        self.use_opus = True  # Use Opus by default
        self.opus_decoder = None  # Opus decoder instance
        self.opus_sample_rate = None  # Sample rate from Opus packets
        self.opus_channels = None  # Channel count from Opus packets
        self.zstd_decompressor = None  # Zstandard decompressor for pcm-zstd
...
class AudioPreviewManager:
    ...
    def start_preview(self, channel: str, instance_id: int, host: str, port: int,
                     ...
                     bandwidth: int = 2700, use_opus: bool = True) -> bool:
        ...
        Args:
            ...
            use_opus: Use Opus compression (default: True), otherwise use pcm-zstd
        ...
        if not ZSTD_AVAILABLE:
            print("Error: zstandard library required for audio support")
            return False
        ...
        ch.use_opus = use_opus and OPUS_AVAILABLE  # Enable Opus if requested and available

        # Initialize decoders
        if ch.use_opus:
            ch.opus_decoder = None  # Will be created on first packet
            print(f"[{ch.channel_name.upper()}] Opus encoding enabled")
        else:
            import zstandard as zstd
            ch.zstd_decompressor = zstd.ZstdDecompressor()
            print(f"[{ch.channel_name.upper()}] PCM-ZSTD encoding enabled")
    ...
    async def _connect_channel(self, ch: AudioChannel):
        ...
        print(f"Connecting {ch.channel_name} channel to {url} (format: {'opus' if ch.use_opus else 'pcm-zstd'})")
        ...
        try:
            async with websockets.connect(url, ping_interval=None) as websocket:
                ...
                # Receive and process audio messages (binary only - JSON deprecated)
                msg_count = 0
                last_debug_time = time.time()

                while ch.running:
                    try:
                        # Check if event loop is still running before waiting
                        if not self.is_running:
                            break
                        message = await asyncio.wait_for(websocket.recv(), timeout=1.0)

                        # Only handle binary messages (Opus or pcm-zstd)
                        if not isinstance(message, bytes):
                            print(f"[{ch.channel_name.upper()}] Warning: Received non-binary message, ignoring")
                            continue

                        # Decode based on format
                        if ch.use_opus:
                            pcm_data, ch.opus_decoder, ch.opus_sample_rate, ch.opus_channels = decode_opus_binary(
                                message, ch.opus_decoder, ch.opus_sample_rate, ch.opus_channels, ch.channel_name.upper()
                            )
                        else:
                            pcm_data = decode_pcm_zstd_binary(message, ch.zstd_decompressor, ch.channel_name.upper())
                        ...
                        # Server sends mono audio for non-IQ modes, but check channel count
                        if ch.use_opus and ch.opus_channels == 2:
                            # Stereo Opus - convert to mono by averaging channels
                            audio_array = audio_array.reshape(-1, 2)
                            audio_array = np.mean(audio_array, axis=1).astype(np.int16)
                        elif ch.use_opus and ch.opus_channels == 1:
                            # Already mono - use as-is
                            pass
                        # For PCM-zstd, data is always mono for non-IQ modes
                        ...
                        if now - last_debug_time >= 5.0:
                            with self.buffer_lock:
                                print(f"[{ch.channel_name.upper()}] Buffer: {len(ch.buffer)} samples, Messages: {msg_count}")
                            last_debug_time = now
                            msg_count = 0
                        ...
                        # Event loop closed or generator exit during shutdown
                        print(f"[{ch.channel_name.upper()}] Shutdown detected: {e}")
                        break
                    except Exception as e:
    ...
    def update_frequency(self, channel: str, frequency: int):
        """Update frequency for a channel using JSON tune command."""
        ch = self.left_channel if channel == 'left' else self.right_channel
    ...
    def update_bandwidth(self, channel: str, bandwidth: int):
        """Update bandwidth for a channel using JSON tune command."""
        ch = self.left_channel if channel == 'left' else self.right_channel
    ...
    def _send_tune_command(self, ch: AudioChannel):
        """Send JSON tune command to update frequency/mode/bandwidth without reconnecting."""
        if not ch.ws or not self.event_loop:
            return

        # Calculate bandwidth edges
        bandwidth_low, bandwidth_high = self._calculate_bandwidth_edges(ch.mode, ch.bandwidth)
        ...
        # Send via WebSocket in event loop
        try:
            import json
            asyncio.run_coroutine_threadsafe(
                ch.ws.send(json.dumps(tune_msg)),
                self.event_loop
            )
        except Exception as e:
            print(f"[{ch.channel_name.upper()}] Error sending tune command: {e}")
